...
import (
	...
	*/
	...
	commands "MIA-PROYECTO2-201612174/commands"
...
func CommandChecker(s *lexmachine.Scanner, consola *string) {
	paramType := ""
...
func controlCommands(command param, consola *string) {
	...
	case "EXEC":
		...
		CommandChecker(ScanInput(input), consola)
		fmt.Println("[-] El archivo ha sido ejecutado con exito.")
	case "PAUSE":
		systemPaused()
	case "MKDISK":
		if requiredParameters([]string{"SIZE", "PATH"}, command) != nil {
			return
		}
		commands.MKDisk(command.path, command.size, command.unit, consola)

	case "RMDISK":
		if requiredParameters([]string{"PATH"}, command) != nil {
			return
		}
		commands.RMDisk(command.path, consola)
	case "FDISK":
		if command.add != "" && command.delete == "" {
			...
			if command.delete == "FAST" {
				commands.FDiskDelete(command.path, false, command.name, consola)
			} else if command.delete == "FULL" {
				commands.FDiskDelete(command.path, true, command.name, consola)
			} else {
			...
			commands.FDisk(command.path, command.size, command.unit, command.Type, command.fit, command.name, consola)
		} else {
			fmt.Println("[ERROR]: El comando FDISK proporciona un error en su estructura")
		}
	case "MOUNT":
		if command.path == "" && command.name == "" {
			commands.ShowMountedDisks(consola)
		} else {
			if requiredParameters([]string{"PATH", "NAME"}, command) != nil {
				return
			}
			commands.Mount(command.path, command.name, consola)
		}
	case "UNMOUNT":
		...
		for _, idn := range command.idn {
			commands.Unmount(idn, consola)
		}
	case "MKFS":
		if requiredParameters([]string{"ID"}, command) != nil {
			return
		}
		commands.Mkfs(command.id, "full", consola)
	case "MKDIR":
		if requiredParameters([]string{"ID", "PATH"}, command) != nil {
			return
		}
		commands.Mkdir(command.id, command.path, command.pathComplete, consola)
	case "MKFILE":
		if requiredParameters([]string{"ID", "PATH"}, command) != nil {
			return
		}
		commands.Mkfile(command.id, command.path, command.pathComplete, command.size, command.txt, consola)
	/*case "CAT":
	...
	case "REP":
		...
		commands.Reports(command.id, strings.ToUpper(command.name), command.path, command.ruta, consola)
	}
}
