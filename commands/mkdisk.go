...
func MKDisk(path string, size int64, unit byte, consola *string) {
	//Se verifica si existe el directorio o si no se crea
	fileName := filepath.Base(path)
	auxPath := strings.TrimRight(path, fileName)
	createDirectory(auxPath, consola)
	fmt.Println(path)
	...
	*consola = *consola + "[-] El disco ha sido creado con exito.\n" +
		"nombre: " + fileName + "\n"
...
func createDirectory(path string, consola *string) {
	_, err := os.Stat(path)
	if os.IsNotExist(err) {
		err := os.MkdirAll(path, 0755)
		if err != nil {
			fmt.Println("Error: El sistema no puedo crear el directorio")
			*consola = *consola + "Error: El sistema no puedo crear el directorio\n"
			return
		}
	}
}
