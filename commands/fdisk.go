...
L: Particoin Logica
*/
func FDisk(path string, size int64, unit byte, typeF byte, fit byte, name string, consola *string) {
	...
	*/
	...
	for i, part := range mbr.Partitions {
		...
		if part.Name == newName {
			...
			*consola = *consola + "[ERROR] Este nombre ya fue asignado a una particion.\n"
			return
		}
	}
	...
	if err != nil {
		...
		*consola = *consola + "[ERROR] la unidad declarada no es valida.\n"
		return
	}
	//[CREACION DE LOGICAS]
	if typeF == 'L' {
		...
		if !existExtended {
			...
			*consola = *consola + "[ERROR] No existe una particion extendida creada.\n"
			return
		}
		...
		if partitionSize <= int64(binary.Size(prevEBR)) {
			...
			*consola = *consola + "[ERROR] El espacio solicitado no es valido para crear una particion lógica.\n"
			return
		}
		...
		if prevEBR.Status == 0 {
			...
			if partitionSize > sizeAvailableExteded {
				...
				*consola = *consola + "[ERROR] El espacio requerido no se encuentra disponible en la particion extendida.\n"
				return
			}
			prevEBR.Fit = fit
		...
		*consola = *consola + "[-] La particion logica ha sido creada con exito.\n"
		return
	}
	//[VALIDACION 1]
	if partitionSize > sizeAvailable {
		fmt.Println("[ERROR] El espacio requerido no se encuentra disponible.")
		*consola = *consola + "[ERROR] El espacio requerido no se encuentra disponible.\n"
		return
	}
	//[VALIDACION 2]
	...
	if index == -1 {
		...
		*consola = *consola + "[ERROR] No existe una particion disponible en el disco.\n"
		return
	}
	...
	*consola = *consola + "[-] La particion ha sido creada con exito.\n"
}

// Funcion para eliminar particiones
func FDiskDelete(path string, full bool, name string, consola *string) {
	//Obtenemos el mbr del disco
	file, mbr, err := readFile(path)
	if err != nil {
		return
	...
	for i, part := range mbr.Partitions {
		...
		if part.Type == 'E' {
			...
			if find == -1 {
				...
				*consola = *consola + "[ERROR]: La particion no fue encontrada.\n"
				break
			}
	...
	*consola = *consola + "[-] La particion" + name + "fue eliminada con exito.\n"
}
