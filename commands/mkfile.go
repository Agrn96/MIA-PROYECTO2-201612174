...
func Mkfile(id string, route string, p bool, size int64, txt string, consola *string) {
	//Revismos que la ruta a insertar sea correcta
	if route[0] != '/' {
		fmt.Println("[ERROR] El path no es valido.")
		*consola = *consola + "[ERROR] El path no es valido.\n"
		return
	}
	...
	if index == -1 {
		...
		*consola = *consola + "[ERROR]: El directorio donde se desea crear el archivo no existe.\n"
		file.Close()
		return
	}
	...
	*consola = *consola + "[-] El archivo \"" + route + "\" ha sido creado con exito.\n"
}
