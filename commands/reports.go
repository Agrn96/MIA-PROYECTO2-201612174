...
func Reports(id string, rep string, path string, route string, consola *string) {
	//Carpeta en donde se crear el reporte
	dir := filepath.Dir(path)
	createDirectory(dir, consola)
	//Reportes de bitmaps
	if strings.HasPrefix(rep, "BM_") {
		reportBM(id, rep, path, route, consola)
		return
	}
	...
	indexSB, name := getPartitionType(mountedPart, consola)
	...
	case "TREE_DIRECTORIO":
		if len(route) == 0 {
			...
			*consola = *consola + "[ERROR]: Necesita especificar una ruta para crear el reporte.\n"
			err = fmt.Errorf("ERROR")
		} else {
			fmt.Println("[REPORT] Creando reporte de TREE_DIRECTORIO.")
			*consola = *consola + "[REPORT] Creando reporte de TREE_DIRECTORIO.\n"
...
func reportBM(id string, rep string, path string, route string, consola *string) {
	//Superboot a trabajar
	sb := superBoot{}
	//Obtenemos el file y la particion a trabajar
	diskPath, mountedPart, err := searchPartition(id)
	...
	//Definimos el tipo de particion que es
	indexSB, _ := getPartitionType(mountedPart, consola)
	//Recuperamos el superbloque de la particion
	sb = getSB(file, indexSB)
