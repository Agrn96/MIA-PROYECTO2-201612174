...
func Mkdir(id string, route string, p bool, consola *string) {
	//Revismos que la ruta a insertar sea correcta
	if route[0] != '/' {
		fmt.Println("[ERROR] El path a crear no es valido.")
		*consola = *consola + "[ERROR] El path a crear no es valido.\n"
		return
	}
	...
	*consola = *consola + "[-] El directorio \"" + route + "\" ha sido creado con exito.\n"
}
...
func getPartitionType(mountedPart mountedParts, consola *string) (int64, string) {
	//Definomos el tipo de particion
	partitionType := typeOf(mountedPart.partition, consola)
